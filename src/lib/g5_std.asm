...
.globl g5_open
g5_open:				# void *g5_open(const char *fName, int flags, int mode);
	sw $ra, -4($sp) # return address
	sw $fp, -8($sp) # frame pointer of caller
	move $fp, $sp # begin new frame
	subu $sp, $sp, 44 # expad frame - expect address of formatter
...
g5_close:				# int g5_close(int);
	...
	syscall						# $v0 = success (0) / failure (-1)
...
.globl g5_read
g5_read:					# int g5_read(int fd, const char *buf, int count);
	sw $ra, -4($sp) # return address
	sw $fp, -8($sp) # frame pointer of caller
	move $fp, $sp # begin new frame
	subu $sp, $sp, 44 # expad frame - expect address of formatter
	...
	lw $a0, 0($sp)		# $a0 = fd (file descriptor)
	lw $a1, -4($sp)		# $a1 = buf
	lw $a2, -8($sp)		# $a2 = count
	li $v0, 14				# read syscall
	syscall						# $v0 = number of bytes read
...
.globl g5_write
g5_write:					# int g5_write(int fd, const char *buf, int count);
	sw $ra, -4($sp) # return address
	sw $fp, -8($sp) # frame pointer of caller
	move $fp, $sp # begin new frame
	subu $sp, $sp, 44 # expad frame - expect address of formatter
	...
	lw $a1, -4($sp)		# $a1 = buf
	lw $a2, -8($sp)		# $a2 = count
	li $v0, 15				# written syscall
	syscall						# $v0 = number of bytes written
